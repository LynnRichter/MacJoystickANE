...
- (id)initWithDevice:(IOHIDDeviceRef)theDevice
{
    ...
    if (self) {
		...
		int productId = [(NSNumber*)IOHIDDeviceGetProperty(device, CFSTR(kIOHIDVendorIDKey)) intValue];
        
        NSLog(@"found device of id %@-%d-%d",productName,vendorId,productId);
        
        
        BOOL ignoreZAxis = YES;
        if ([productName isEqualToString:@"Logitech Dual Action"])
            ignoreZAxis = NO;
        ...
        for (i=0; i<elements.count; ++i) {
            ...
            if (elementUsage == kHIDUsage_GD_Hatswitch) {
                ...
                if (!(elementUsage == kHIDUsage_GD_X || elementUsage == kHIDUsage_GD_Y || elementUsage == kHIDUsage_GD_Rx || elementUsage == kHIDUsage_GD_Ry || (!ignoreZAxis && (elementUsage == kHIDUsage_GD_Z || elementUsage == kHIDUsage_GD_Rz) ))) {
                    NSLog(@"Potential axis skipped with usage: %d",elementUsage);
                    continue;
                }
...
- (double)getRelativeValueOfAxesIndex:(int)index {
    ...
    value = ((double)IOHIDValueGetIntegerValue(pValue)-min) * (2/(max-min))-1.0;
